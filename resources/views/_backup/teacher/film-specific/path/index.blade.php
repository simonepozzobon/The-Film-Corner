          ...
          @foreach ($apps as $key => $app)
            <div id="{{ $app->slug }}" class="box {{ $app->colors }} {{ $key == 0 ? '' : 'mt' }}">
              <div class="box-header">
                ...
                <div class="btns">
                  @if ($app->available == 1)
                    <a href="{{ route('teacher.film-specific.app', [$app_category->slug, $app->slug]) }}" class="btn btn-{{ $app->colors }}" data-toggle="tooltip" data-placement="top" title="{{ GeneralText::field('new') }}">
