...
@section('content')
  <div class="container">
    @include('components.apps.heading', ['route' => route('student'), 'app_category' => $app_category, 'apps' => $apps])
    <div class="row mt">
          ...
          @foreach ($apps as $key => $app)
            <div id="{{ $app->slug }}" class="box {{ $app->colors }} {{ $key == 0 ? '' : 'mt' }}">
              <div class="box-header">
                ...
                <div class="btns">
                  @if ($app->available == 1)
                    <a href="{{ route('student.creative-studio.app', [$app_category->slug, $app->slug]) }}" class="btn btn-{{ $app->colors }}" data-toggle="tooltip" data-placement="top" title="{{ GeneralText::field('new') }}">
...
<script type="text/javascript">
      ...
      success: function (response) {
        console.log(response);
          ...
          for (var i = 0; i < dataLenght; i++) {
            ...
            data +=       '<a href="/student/creative-studio/'+response.category+'/'+response.app['slug']+'/'+response.sessions[i]['token']+'" class="btn btn-'+color+'"><i class="fa fa-folder-open-o" aria-hidden="true"></i> {{ GeneralText::field('open') }}</a>';
            data +=       '<a href="#" id="share-session" onclick="shareSession('+studentId+', '+appId+', \''+response.sessions[i]['token']+'\')" class="btn btn-'+color+'"><i class="fa fa-share-alt" aria-hidden="true"></i> {{ GeneralText::field('share') }}</a>';
  ...
  function deleteSession(sessionToken)
  {
    ...
    $.ajax({
      type: 'POST',
      url:  '{{ route('student.session.delete') }}',
      headers: {
        'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
      },
      data: formData,
