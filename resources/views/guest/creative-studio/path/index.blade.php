          ...
          @foreach ($apps as $key => $app)
            <div id="{{ $app->slug }}" class="box {{ $app->colors }} {{ $key == 0 ? '' : 'mt' }}">
              <div class="box-header">
                ...
                <div class="btns">
                  @if ($app->available == 1)
                    <a href="{{ route('guest.creative-studio.app', [$app_category->slug, $app->slug]) }}" class="btn btn-{{ $app->colors }}" data-toggle="tooltip" data-placement="top" title="{{ GeneralText::field('new') }}">
                    ...
                    <div class="modal fade" id="fullModal" tabindex="-1" role="dialog" aria-labelledby="fullModalWarning" aria-hidden="true">
                       <div class="modal-dialog modal-lg" role="document">
                         <div class="modal-content">
                           ...
                           <div class="modal-footer">
                             <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fa fa-times" aria-hidden="true"></i> {{ GeneralText::field('close') }}</button>
                           </div>
                         </div>
                       </div>
                     </div>
      ...
      success: function (response) {
        console.log(response);
          ...
          for (var i = 0; i < dataLenght; i++) {
            ...
            data +=       '<a href="/guest/creative-studio/'+response.category+'/'+response.app['slug']+'/'+response.sessions[i]['token']+'" class="btn btn-'+color+'"><i class="fa fa-folder-open-o" aria-hidden="true"></i> {{ GeneralText::field('open') }}</a>';
