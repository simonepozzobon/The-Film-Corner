          ...
          @foreach ($apps as $key => $app)
            <div id="{{ $app->slug }}" class="box {{ $app->colors }} {{ $key == 0 ? '' : 'mt' }}">
              <div class="box-header">
                ...
                <div class="btns">
                  @if ($app->available == 1)
                    <a href="{{ route('guest.creative-studio.app', [$app_category->slug, $app->slug]) }}" class="btn btn-{{ $app->colors }}" >
      ...
      success: function (response) {
        console.log(response);
          ...
          for (var i = 0; i < dataLenght; i++) {
            ...
            data +=       '<a href="/guest/creative-studio/'+response.category+'/'+response.app['slug']+'/'+response.sessions[i]['token']+'" class="btn btn-'+color+'"><i class="fa fa-folder-open-o" aria-hidden="true"></i> Open</a>';
            data +=       '<a href="#" id="share-session" onclick="shareSession('+guestId+', '+appId+', \''+response.sessions[i]['token']+'\')" class="btn btn-'+color+'"><i class="fa fa-share-alt" aria-hidden="true"></i> Share</a>';
