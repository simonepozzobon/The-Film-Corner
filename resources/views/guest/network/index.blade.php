          ...
          <div class="box {{ $item->colors[0] }} mt">
            ...
            <div class="box-body">
              ...
              <h6 class=""><span class="badge badge-dark-{{ $item->colors[0] }}">{{ $item->app_name }}</span></h6>
                <p>{{ substr(strip_tags($item->notes), 0, 200) }}{{ strlen(strip_tags($item->notes)) > 200 ? '...' : "" }}</p>
            </div>
            <div class="box-body">
              <network-icons
                  views="{{ $item->views }}"
            ...
            <div class="box-btns">
              <a href="{{ route('guest.network.single', $item->token) }}" class="btn btn-block btn-{{ $item->colors[0] }}">View</a>
            </div>
          </div>
        </div>
      @endforeach
