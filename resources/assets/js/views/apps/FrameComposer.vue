<template>
    <app-template :app="app">
        <template slot="left">
            <ui-app-preview ref="preview" :title="$root.getCmd('character')" />
        </template>
        ...
        <template>
            ...
            />
            <ui-app-note class="mt-4" @changed="setNotes" :initial="notes" />
        </template>
    </app-template>
</template>
    ...
    methods: {
        ...
        getCanvasSize: function(hasReturn) {
            if (this.$refs.preview) {
                ...
                elSize.hClean =
                    elSize.hClean - titleSize.hClean - titleSize.marginY;
                this.size = elSize;
                if (hasReturn) {
                    return this.size;
                }
            ...
            let image = new fabric.Image.fromURL(url, (obj, opts) => {
                ...
                if (fromOpen && savedObj) {
                    ...
                    if (libraryID != 1) {
                        ...
                        if (width < this.canvasWidth) {
                            scaleFactor =
                                (this.canvasWidth * scaleFactor) / width;
                            this.landscape.set({
                                scaleX: scaleFactor,
                                scaleY: scaleFactor
                            });
