...
{
  public function openSessions($guest_id, $app_id)
  {
    $sessions = GuestAppSession::where([
      ['guest_id', '=', $guest_id],
      ['app_id', '=', $app_id]
    ])->get();
  ...
  public function updateSession(Request $request)
  {
    ...
    $session = GuestAppSession::where('token', '=', $request['token'])->first();
    $session->is_empty = 0;
  ...
  public function shareSession(Request $request)
  {
    ...
    $session = GuestAppSession::where('token', '=', $request['token'])->with('guest', 'app')->first();
    $guest = $session->guest()->first();
    $app = $session->app()->first();
  ...
  public function approveSession(Request $request)
  {
      ...
      return redirect()->route('guest.settings.index');
  }
